...
async function runFundingArbInner(): Promise<any> {
  ...
  try {
    ...
    function arbNotEval(label: string, hint?: string): import("@core/types.mts").DecisionGate {
      return { label, passed: false, actual: "not evaluated", required: "—", hint };
    }
    type Gate = import("@core/types.mts").DecisionGate;
    ...
    for (const coin of ARB_COINS) {
      ...
      const entryDecision: import("@core/types.mts").EntryDecisionSnapshot = {
