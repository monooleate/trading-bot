...
const SCHEMA: Record<string, FieldSpec> = {
  ...
  betaCapFraction: { default: 0.25, min: 0.05, max: 1.0, label: "Crypto-beta cap fraction (crypto+HL)",  step: 0.01, unit: "frac", category: "common", group: "Portfolio risk", help: "A kombinált (crypto + HL) bankroll hányada, amennyi crypto-directional tőke egyszerre lekötve lehet. 0.25 = 25%. A discovery ~15-20%-ot javasol; 0.25 óvatosan generózus. Csak akkor hat, ha a cap ON." },
  directionalHalt: { default: 0,   min: 0,    max: 1,   label: "Directional halt (measure-only)",  step: 1,    unit: "bool", category: "common", group: "Portfolio risk", help: "ON (B74): a bot NEM nyit új directional pozíciót — se crypto up-or-down, se HL perp —, de a ledgerbe tovább logolja a predikciókat (a mérés folytatódik, torzítatlan). A tiszta ledger 2026-09-14-én ezeket a piac ára ALATT árazta (crypto up-or-down −44% n=72, HL −59% n=31, a CI a 0-t nem tartalmazza). A crypto THRESHOLD (above-K) piacokat NEM érinti — az az egyetlen crypto ág, aminek van támogatása. OFF (default): nincs változás." },
  // ─── Risk overlays (model-discovery-expansion §4.C / B49 #8) ──
  riskVolTargetEnabled: { default: 0,    min: 0,    max: 1,   label: "Vol-target sizing (crypto only)",       step: 1,    unit: "bool", category: "common", group: "Portfolio risk", help: "ON: a ¼-Kelly méretet a (cél-vol / realizált-vol) aránnyal skálázza (clamp) — csökkenti a méretet, ha a bot realizált hozam-volatilitása felpörög, visszaállítja, ha nyugodt. Robusztus tail-kontroll. OFF (default): nincs skálázás." },
  riskVolTarget:        { default: 0.10, min: 0.02, max: 0.5, label: "Target per-trade vol",     step: 0.01, unit: "frac", category: "common", group: "Portfolio risk", help: "A cél per-trade hozam-volatilitás (szórás). A realizált vol az utolsó 30 lezárt trade pnl%-ából. Csak akkor hat, ha a vol-target ON." },
  riskDdKillEnabled:    { default: 0,    min: 0,    max: 1,   label: "Drawdown kill-switch (crypto only)",    step: 1,    unit: "bool", category: "common", group: "Portfolio risk", help: "ON: leállítja az ÚJ belépőket, amint a peak-to-current equity drawdown eléri a limitet (a nyitott pozíciókat nem érinti). Elvi peak-equity stop a bruttó-veszteség odométer helyett (vö. B33). OFF (default): nincs DD-stop (a meglévő sessionLossLimit marad). ⚠ **HATÓKÖR (audit P2-11): csak a CRYPTO runnerbe van bekötve**, a `common` kategória ellenére — a weather (élőben −31,9% drawdown mellett is kereskedik), a HL, az F-Arb és a sports NEM védett. ⚠ **Paper módban `paperNeverStop`=1 mellett csak NAPLÓZ, nem blokkol**: a `peak` monoton a bankrollStart-tól, tehát a limit átlépése után csak nyerő trade vinné vissza alá — amit maga a kapu tilt. Egyirányú ajtó, permanens leállás. LIVE-ban változatlanul kemény stop." },
  riskMaxDdFraction:    { default: 0.25, min: 0.05, max: 0.8, label: "Max drawdown fraction",    step: 0.01, unit: "frac", category: "common", group: "Portfolio risk", help: "A megengedett maximális peak-to-current drawdown (a csúcs-equity hányada). 0.25 = 25%. Felette a kill-switch (ha ON) blokkolja az új belépőket." },
  ...
  weatherSigmaInflation:     { default: 1.0,  min: 1.0, max: 4.0, label: "Forecast σ inflation (P0-1)", step: 0.25, unit: "×",     category: "weather", group: "Risk & sizing", help: "A bucket-matcher-nek átadott előrejelzési σ szorzója. A GEFS ensemble szórása csak a SAJÁT perturbációit méri, a strukturális modellhibát nem látja: az EMOS-store FORWARD felén (élő METAR obs vs. a ténylegesen használt GEFS-átlag, n=35) mean(err²/σ²) = 10.04, ahol az 1.0 lenne a kalibrált — vagyis a σ ~1.8× (medián, outlier-robusztus) … ~3.2× (átlag) túl kicsi. Következmény nem rossz IRÁNY, hanem téves MAGABIZTOSSÁG: σ=0.5 mellett egy μ-től 1.55 °C-ra lévő bucket ~1.6%-ot kap, és a bot erre a kvázi-bizonyosságra méretez ¼-Kelly-t. A mért plató **2.25** az ÉLŐ úton (ahol `weatherUseEmos=1`; nyers ensemble-on 2.0–2.5) — `selectPlateau`-val választva, nem csúcs-illesztéssel. Az EMOS a var-ratio-t 10.04 → 5.96-ra viszi, a maradék ~6× túl-magabiztosságot ez a szorzó tünteti el (λ=2.25 → var-ratio 1.18). ⚠ KÁR-CSÖKKENTÉS, NEM EDGE: monoton transzformáció, tehát a corr(predikció, kimenet) = −0.316 (n=28) előjelét NEM javítja — csak abbahagyatja a botal, hogy erre fogadjon. A 28 valós belépőn szimulálva: Brier 0.3513 → 0.2969, realizált PnL −$11.82 → −$2.96 (de a Brier így is 0.25 FÖLÖTT marad, tehát nyereségessé nem teszi). 1.0 = KI (bit-azonos, default)." },
  weatherEmosSeedWeight:     { default: 1.0,  min: 0.01, max: 1.0, label: "EMOS seed residual weight (P0-2)", step: 0.01, unit: "w", category: "weather", group: "Risk & sizing", help: "A SEEDELT (historikus backfill) EMOS-residualok súlya az illesztésben. 1.0 = KI (bit-azonos a mai fittel). Indok: minden élő állomás ~181 seedelt (ERA5 obs + inter-modell szórás) és 0-5 forward (METAR obs + GEFS σ) residualon illeszkedik — KÉT KÜLÖNBÖZŐ ELOSZLÁS: a seed bias −0,03 °C, a forwardé +0,50 °C. A seedelt átlag-korrekció ezért NEM transzferál, viszont ~40:1 arányban leszavazza az élő adatot. Leave-one-out mérve a 35 forward residualon (a kihagyott pontot az illesztés NEM látja): súly 1.0 → CRPS 1,0767 · log-score 3,5778; súly 0.1 → 0,9681 · 3,1933; súly 0.03 → 0,8623 · 2,9527. ⚠ PÁRBAN ÁLL a `weatherSigmaInflation`-nel: a jobban illesztett EMOS KEVESEBB utólagos σ-tágítást kíván (seed 1.0 → λ 2,25–2,5; seed 0.1 → λ 1,75–2,25; seed 0.03 → λ 1,5–2,0). Ne hangold az egyiket a másik újramérése nélkül. **Ajánlott pár: seed 0.1 + λ 2.0.** ⚠ 0.03-nál az effektív mintaméret ~10-re esik állomásonként — a mért nyereség valós, de nagy szórású; a 0.1 a konzervatív beállítás." },
  weatherShrinkSizing:       { default: 0,    min: 0,   max: 1,   label: "Apply selection shrink to sizing (P2-12)", step: 0.25, unit: "frac", category: "weather", group: "Risk & sizing", help: "0 = KI (default, bit-azonos). A `weatherSelectionShrink` optimizer-curse büntetése ma CSAK a trade/nem-trade kaput mozgatja — a Kelly utána a NYERS valószínűségre méretez. Ez belsőleg inkonzisztens: ha elhisszük, hogy a nyertes bucket edge-e felfelé torzított (mert N közül a maximumként választottuk), akkor nem szabad a teljes torzított konvikciót megtenni rá. Egy épp hogy átcsúszó trade ma teljes mérettel megy. 1 = a teljes büntetést levonjuk a fogadott oldal valószínűségéből is. ⚠ Ne keverd: az operátor gyakran a `weatherSelectionShrink`-től várja a méret-csökkentést — az viszont csak a GYAKORISÁGOT állítja; a méret-knob a `weatherKellyScale`." },
  ...
  weatherMultiModelRecord:   { default: 1,    min: 0,   max: 1,   label: "Multi-model log-forward (B52 #1)", step: 1, unit: "bool", category: "weather", group: "Forecast pipeline", help: "ON (default): minden ~3 órában állomás+dátumonként lekéri a 4 független ensemble-rendszert (NCEP GEFS 31 + ECMWF IFS-ENS 51 + ECMWF AIFS-ENS 51 + Google WeatherNext 2 64 = 197 tag, EGYETLEN kérésben, kulcs nélkül) és eltárolja, mit mondott mindegyik — a bot által ténylegesen használt GEFS-only μ/σ mellé. Az obs a METAR-ból jön (az EMOS-reconcile már lekérte, nincs extra hívás). **Nulla trading-hatás** — semmi nem olvassa vissza a trade-útra. Azért default-ON, mert a összehasonlítás NEM visszatölthető: az Open-Meteo historical API a WeatherNext 2-t csak ~2026-09-04-től adja, az AIFS-ENS-nek nincs archívuma. OFF: nem gyűlik adat, a flip-döntés (weatherUseMultiModel) bizonyíték nélkül marad." },
  weatherUseMultiModel:      { default: 0,    min: 0,   max: 1,   label: "Multi-model ensemble (μ,σ) — B52", step: 1, unit: "bool", category: "weather", group: "Forecast pipeline", help: "OFF (default): a bucket-matcher μ/σ-ja a mai 31-tagú GEFS-ből jön — bit-azonos a korábbi viselkedéssel. ON: a 4-rendszeres, MODELLENKÉNT egyenlő súlyú keverék hajtja a μ-t és a σ-t (σ² = modellen belüli variancia átlaga + a modell-átlagok varianciája). Mért indok (2026-09-08, a bot állomásain, T+0/T+1): a GEFS-only σ 2-3×-osan alulbecsli a bizonytalanságot Ázsiában/USA-ban (Tokyo T+0: σ 0.49 → 1.58, μ 32.3 → 29.3), Európában viszont szűkebb — nem tágítás, hanem helyesebb σ. **Várható mellékhatás:** a nagyobb σ lejjebb viszi a confidence-t (1 − σ/4), így a weatherConfidenceMin kapu több piacot blokkol — ez szándékos. CSAK a weatherMultiModelRecord által gyűjtött head-to-head pozitív eredménye után kapcsold be." },
  ...
  sportsCronEnabled:         { default: 1,    min: 0,   max: 1,   label: "Sports cron enabled", step: 1, unit: "bool", category: "sports", group: "Risk & sizing", help: "1 = a sports bot fut a 3 perces worker-tickben (mai viselkedés). 0 = a cron-tick kihagyja; a manuális Scan és a többi action változatlanul működik. **Miért kell (audit P2-9):** eddig a bot leállításának EGYETLEN módja a `session.stopped` flag volt — azt viszont a reset, a simVersion-bump és egy átmeneti olvasási hiba is eltörli. Pontosan ez történt: a 2026-07-23-i operátor-döntés (a sports maradjon leállítva, amíg nincs Pinnacle odds-feed — B37) a Phase-4 tiszta indulással **csendben elveszett**, és a bot 2026-09-09 06:01-kor újra pozíciókat nyitott. A cron-knob **tartós** operátor-szándék: reset és séma-bump nem törli. ⚠ Emlékeztető: a sports fair-value-ja bizonyítottan (n=161, 0 eltérés) `0.5 + 0.55·(ár − 0.5)`, azaz annak az árnak determinisztikus transzformációja, ami ellen fogad → nulla forecasting-információ, amíg a B37 odds-feed nincs bekötve." },
