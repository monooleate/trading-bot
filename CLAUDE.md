...
### Párhuzamos munka (2026-09-10, a 93. sessionnel egy időben) — API-szolgáltatás felmérés (read-only) → most nem; ⚠ a box nem nyithat Polymarket-pozíciót
...
> **✅ B74 élesítve (2026-09-17 14:28 UTC, az operátor futtatta a SQL-t — a saját DB-írásomat a harness classifier blokkolta):** `directionalHalt=1`, **18 override**. Élőben verifikálva: a workerek tickelnek és olvassák a knobot, az élesítés óta 0 új pozíció nyílt. A halt-log egyelőre 0 — helyes: a gate csak azon a piacon tüzel, ami különben belépett volna, és a directional ágakat a korábbi kapuk amúgy is kiszűrik; az első valódi directional belépési kísérletnél fog „Directional halt (B74)"-t logolni. A threshold crypto ág és a mérés (ledger) változatlanul fut.
...
**B38 — előkészítve, NEM implementálva.** A (3) alpont (korreláció-tudatos cap) **nagyrészt lefedve**: a B49 #2 `checkBetaCap` él (0.25) és a crypto+HL **együttes** kitettséget capeli — tágabban, mint amit a B38 kért; a maradék rés a **rezolúciós-ablak-szegmentálás**. A `btcMinPriceBand=0.10` aktív, a `maxEdgeCap` nyitva. Friss adat: mind az 5 crypto trade YES 0.10–0.38-on, mind 0.000-ra rezolvált — **n=5, kevés** live-feature-höz. Terv: (1) előbb ledger-szimuláció (a B56b tanulsága: plauzibilis javítás mérve **ronthat**), (2) precondition ≥30 **tiszta** crypto sor (ma 1 van az egész rendszerben), (3) csak azután knob default-0-val, (4) a döntést a promóciós kapu hozza.

**Napi drift-ellenőrzés beállítva** (`edgecalc-drift-check`, 08:00 helyi, lokális ütemező — **nem** felhő-routine, mert annak nincs SSH-ja a boxhoz, tehát pont az élő állapotot nem látná). Nyolc read-only check: knobok vs. CLAUDE.md · session-állapotok (F-Arb a `blob_kv`-ben!) · bankroll-ugrás · **tiszta first-obs számláló** (ma 1, nőnie kell) · `codeVersion` vs. box `BUILD_INFO` · recorder-frissesség · deploy-szinkron · konténer-egészség. Csak eltérést jelent. **Napi, amíg stabilnak nem bizonyul, utána heti.** Doksi: [changelog](internal-docs/changelog/CHANGELOG-2026-09-09.md) · [sprints B36/B38](internal-docs/roadmap/sprints.md).

### Korábbi munka (90. session, 2026-09-09) — TELJES RENDSZER-AUDIT a charta szerint → 16 lelet, 15 javítva

A user a [`playbooks/system-audit.md`](internal-docs/playbooks/system-audit.md) szerinti teljes auditot kérte, majd a javításokat prioritási sorrendben. **1. fázis read-only, adat-first** (élő Postgres + konténer-logok + valós Gamma/Open-Meteo válaszok; a kód csak a mért jelenség MAGYARÁZATÁRA). **Eredmény: P0×3, P1×5, P2×5, P3×3** — mindegyik mérés + `fájl:sor` + ellen-hipotézis. **2. fázis: 8 commit**, mindegyik `tsc` 0 + teljes suite + build zöld; minden viselkedés-változás **default-OFF**; teszt 49 → **52**.

**A 3 P0.** **(1) Weather σ 1,8–3,2× túl kicsi** — a forward EMOS-residualokon (élő METAR obs vs. a ténylegesen használt GEFS-átlag, **n=35**) `mean(err²/σ²)` = **10,04**, ahol 1,0 a kalibrált; a hatás nem rossz irány, hanem téves magabiztosság: a belépéskori (B53-mentes) adaton a `[0–0,15)` predikció-sáv (n=7, átlag 0,104) **71,4%-ban YES**-re rezolvált és **−$20,68**-t vitt a −$11,82-os összesből → új `weatherSigmaInflation` (default 1.0 = KI, mért ajánlás **2.25**), [math/38](internal-docs/math/38-weather-dispersion.md). **(2) EMOS seed-dominancia 4887:35** — a seed (ERA5 + inter-modell szórás, bias −0,03 °C) ~40:1-ben leszavazza az élőt (METAR + GEFS σ, bias **+0,50 °C**) → súlyozott OLS + `weatherEmosSeedWeight` (default 1.0 = KI, ajánlás **0.1**), [math/23 §6](internal-docs/math/23-emos.md). **(3) A HL perp-iránya threshold-piacból jött** — a kereső puszta „bitcoin" kulcsszóra esett vissza, és **2412 egymás utáni scanben** a `bitcoin-above-82k` (YES 0,0365, mély OTM) `p≈0,0156`-ját `|p−0,5|×2`-vel **96,9% hamis edge**-dzsé alakította, amit a 40%-os cap blokkolt → **a bot 09-05 óta képtelen volt kereskedni**, és a LONG-bias (B18) forrása is ez volt (az irányt a Polymarket volumen-rangsora döntötte el). Élőben verifikálva: BTC most `bitcoin-up-or-down`, **az ETH egyáltalán kap jelet**, a SOL helyesen leáll.

**A 5 P1.** A **B53 back-fill „mosott" adatot** ad ki elsőnek (36 latchelt sorból **32-t** már >10× scanneltek; `hyperliquid/BTC` firstMarketPrice 0,9995 vs outcome 1) → új `firstBackfilled` provenancia-jelzés + UI-figyelmeztetés a walk-forward kártyán. A **HL realized-IC strukturálisan 0** volt (a `pnlUSDC` vs `pnl` mező-eltérés miatt minden trade veszteségnek címkézve; 8 jel × pontosan 0,0 **nem mérés, hanem törött cső**) + új „degenerate" jelzés, ami megkülönbözteti a „mértük, nincs skill"-t a „nem tudtuk mérni"-től. A **`useRealizedIC` dokumentált defaultja soha nem érvényesült** (nyers override-map + `=== 1`). A **B55 sports-kizárása félig** volt alkalmazva (a promóciós kapu két összeférhetetlen poolt kevert). A **HL-ledger 2 halhatatlan sora** 2879 scanre → piacra kulcsolva + `reconcileLedger` bekötve.

**⚠ Két eset, ahol a mérés megfordította a saját narratívámat.** A P0-2 audit-megfogalmazása **részben téves volt**: az EMOS forward adaton **javít** (var-ratio 10,04 → 5,96); csak a **torzítást** nem viszi el. És a σ-szorzó **path-függő**: nyersen 2,0–2,5, az élő (EMOS-be) úton **2,25** — az élő út **kevesebb** inflációt kér.

**⚠ Amit a P0-1 NEM old meg.** A σ-tágítás monoton, ezért a `corr(predikció, kimenet)` = **−0,316** (n=28) előjelét nem fordítja meg, és a Brier **minden λ-nál 0,25 fölött marad** — **kár-csökkentés, nem edge**. A naiv „flippeljük meg" szintén nem működik (Brier(1−p) = 0,2753) — ugyanaz a csapda, mint a B56b-nél.

**Operátor-jóváhagyás után elvégezve (ugyanebben a sessionben):** a 9 commit **`main`-re pusholva → CI + Deploy zöld**, az új kód fut a boxon (11:03). **A HL-fix élőben igazolva:** a 7 napig capelt `BTC SHORT finalProb 0.0156 / edge 96,9%` helyett most `BTC LONG 0.5198 / edge 4,0%` és **`ETH LONG 0.5254 / edge 5,1%`** — az ETH-nek először van egyáltalán jele. **A sports leállítva** (`sportsCronEnabled=0` + manuális session-stop, olyan indoklással, amit a `paperNeverStop` nem old fel) — ⚠ közben kiderült és javítva lett, hogy a `stopped` ág **a rendezés ELŐTT** tért vissza, tehát a leállítás bennragasztotta volna a 3 nyitott pozíciót ($7,50); most a settlement a stop- és cron-kapu elé került. **A 3 knob élesítve** (`weatherSigmaInflation` 2.25, `weatherEmosSeedWeight` 0.1, `sportsCronEnabled` 0), DSR-trial naplózva. **Hátra:** P3-16 log-megőrzés (infra-döntés, backlog). **+ B66 (peer-lelet, ugyanaznap):** a ledger nem rögzítette, **melyik KÓD** készítette a predikciót — a `configHash` a knobokat hasheli, nem a kódot, tehát egy attribúciós arm némán átível minden knob-változás nélküli deployt (élőben az `e03b4835` arm tartalmazta a 11:03-as audit-deployt). Új `codeVersion` + `codeVersionSpread` (a SHA-t **nem** hasheltem a configHash-be: minden push új armot nyitna). Deployolva, élőben igazolva: a `3683673b` és `e03b4835` armok **mixed**-ként jelennek meg. ⚠ Közben elrontottam a deploy-workflow YAML-t (escape-hiba), 10 percen belül javítva (`66efbf6`) — éles hatás nem volt, a box a korábbi jó deployt szolgálta ki. Doksi: [sprints B58–B65](internal-docs/roadmap/sprints.md) · [changelog](internal-docs/changelog/CHANGELOG-2026-09-09.md).

### Korábbi munka (89. session, 2026-09-09) — B57: coin-diverzifikált crypto scan-slotok, ablak 3 → 5

A user a threshold-fókuszt kérte, és hogy „mi kell, hogy a BTC is szállítsa, amit az ETH/SOL már szállít". **A premissza fordítva volt:** a B51-deploy óta eltelt 26 órában a ledger **BTC threshold 25 sor, ETH threshold 1, SOL 0** — az ETH/SOL **soha nem került a scan-ablakba**. Ok: `markets.slice(0, 3)` a [`pillars/index.mts`](services/worker/src/pillars/index.mts)-ben, nyers 24h-volumen szerint. Élő Gamma-rangsor: **171 crypto piac, 154 threshold** (BTC 77 · ETH 44 · SOL 33; up-or-down mindössze 6), és a **BTC birtokolja az első ötöt** (ETH legjobbja #6, SOL a top 12-ben sincs).

**Implementálva** (`tsc` 0 + **49/49 teszt** + build zöld): új pure [`@core/scan-slots.mts`](packages/core/src/scan-slots.mts) `selectScanSlots` — a volumen marad az elsődleges rangsor (likviditás-proxy a B49 #1 fill-modellnek), de **minden jelen lévő coin kap egy fenntartott slotot**, a maradékot volumen tölti; round-robin tartalék-osztás; egyetlen coin / `minPerCoin=0` esetén **bit-azonos a régi `slice(0,N)`-nel**. [24 assert](packages/core/src/scan-slots.test.mts). **Ablak 3 → 5**, új knob **`cryptoScanWindow`** (default 5, 1–12) + env `CRYPTO_SCAN_WINDOW` — azért Settings-tunable, mert **ez a bot fő külső-API tárcsája** (minden slot egy teljes combiner-futás ~8 fetch-csel; 3 → 5 ≈ **+67% hívás**). **Élő szimuláció:** RÉGI `BTC:82k, 74k, 72k` → ÚJ `BTC:82k, 74k, 72k, ETH:2600, SOL:110` — a 3 BTC slot **változatlan**, nulla regresszió.

**Szándékosan NEM:** „okosabb" rangsor a volumen helyett (a saját #3 javaslatom) — bizonyíték nélküli heurisztika, pont amiről a B56b kimutatta, hogy árt; SOL-kizárás a gyengébb σ miatt (nincs Deribit-opciója) — a confidence/resolution-risk kapuk kezelik, a logolt sor pedig adat.

**⚠ Őszinte korlát:** a threshold-ág a base rate-et veri (+90.9%, n=20), **de a piaci árat nem** (Brier 0.0226 vs 0.0166; a nem-konvergált 17 soron −23.9%). A baseline B53-szennyezett, tehát a valós szám jobb — de **bizonyítottan pozitív edge nincs**. A bővítés indoka a **bizonyíték-gyűjtés üteme**, nem ismert profit. Doksi: [sprints B57](internal-docs/roadmap/sprints.md) · [changelog](internal-docs/changelog/CHANGELOG-2026-09-09.md).

### Korábbi munka (88. session, 2026-09-09) — a B56 javaslatok LEMÉRVE → #2 elvetve, `combinerLogOddsStrength` visszavonva
...
**Mechanizmus (kódból):** a threshold-ágnak **strukturális horgonya** van (`combinerKAnchorStrength`=1.0 → a `vol_divergence` BS-digitális fair-value-ja); a directional ágnak **nincs** — 9 gyenge, egymásnak ellentmondó jel súlyozott átlaga matematikailag 0.5 köré esik, és a `combinerLogOddsStrength=1` ott aktív ugyan, de ellentmondó logitok átlaga is 0 marad. **Nem a pooling romlott el.** Ráadás: a `cond_prob` (`|s−0.5|`=0.001), `funding_rate` (0.002) és `oi_delta` (0.007) gyakorlatilag **konstans 0.5**, együtt a pool-súly **~32%-a** → mechanikusan a 0.5 felé húznak. **A 5 megkötött trade** mind YES 0.10–0.38 belépőn, mind 0.000-ra rezolvált → longshot-aláírás; a forecast jó, a **szelekció/méretezés** a hibás (optimizer's curse — a cryptónak nincs `selectionShrink`-je, a weathernek van). Jelölt lépések: [sprints B56](internal-docs/roadmap/sprints.md).

**Júliusi knobok visszaállítva** (a Phase-4 tiszta indulás a history mellett az override-okat is eldobta; a boxon a `.env` egyetlen bot-tuning változót sem tartalmaz): `weatherSelectionShrink` **1.0**, `weatherMaxPositionUSD` **15**, `frMinSpreadHourly` **0.00005**, `sportsSessionLossLimit` **50** + `sportsSessionLossLimitEnabled` **1**, `sessionLossLimit` **1000** — a meglévő 10 knob megőrizve, `appendTrial` a DSR-nek. **Két knob szándékosan NEM állt vissza:** `weatherInvertDirection` (a júliusi döntés IC=−0.359-nél született, azóta **+0.393** → bekapcsolva egy bizonyítottan jó irány ellen fogadna; B40) és `combinerKBlindDownweight`=0.5 (kizárólag a threshold-ágra hat — arra, amiről ma derült ki, hogy a bot **egyetlen működő fele**, és a +90.9%-ot a jelenlegi 1.0 mellett mérte) → **operátor-döntést kér.**

### Korábbi munka (84-86. session, 2026-09-09) — a knob-kiértékelés → P1 mérési hiba (B53 fix) + B52 4. lépcső (8-rendszeres ensemble)

**84. (read-only):** a user a 09-04-én bekapcsolt 10 knob Edge-Tracker-kiértékelését kérte. **A kiértékelés NEM elvégezhető** két független okból: **(1) nincs pre-flip kar** — a config-fingerprint jól elválasztja a 09-03 22:09-es flippet, de `101cde4d` (flip előtt) **n=3** vs `3683673b` (utána) **n=214**, mert a ledger rescan-enként **felülírta** a config-stampet; a Thompson-bandit „101cde4d 58% prob-best"-je a priorból jön (`nEff=0.4`), nem bizonyíték. **(2) a metrika maga sérült** → B53. **Amit a szennyezéstől függetlenül tudunk:** a crypto modell **nem-informatív** — átlagos `|p−0.5|` = **0.119** vs a piac **0.296**, Brier 0.213 vs a triviális „mindig 0.5" 0.25 → a combiner a 0.5 körül lebeg, míg a piac határozottan és helyesen áraz (a Sprint 41-42A „lapos finalProb" patológia, **nem** knob-kérdés) → **B54**.

**85. (implementálva,** `tsc` 0 + **48/48 teszt** + build zöld**):**

**B53 — a mérési réteg rossz mezőt olvasott.** A [`prediction-ledger`](packages/core/src/prediction-ledger.mts) minden rescan-nél frissítette a `predictedProb`/`marketPrice`/`configHash`-t, így egy rezolvált sor az **utolsó** scan állapotát hordozta — amikorra a piaci ár már mechanikusan a kimenetre konvergált. Mérve: a weather ledger 43 rezolvált sorából **17-nek az ára > 0.98** (ezek **94%-a** YES-re rezolvált), 18 ára a kimenettől 0.02-n belül; a szennyezettség sorrendje (weather 17/43 → −233.6%, crypto 4/73 → −98.7%, sports 1/118 → −0.6%) pontosan megmagyarázza a „rosszaság" sorrendjét. **Fix:** új **írás-egyszer** hármas (`firstPredictedProb`/`firstMarketPrice`/**`firstConfigHash`**), az `upsertRecords` a frissítés **ELŐTT** latch-el (így pre-B53 rekordnál a régebbi értéket veszi); a „latest" mezők maradnak a UI-nak. A config szándékosan a hármas része: a predikciót ahhoz a confighoz kell kötni, amelyik **készítette** — ez oldja fel a B54 kar-kiéheztetését. **4 fogyasztó** vált `?? latest` fallbackkel (walk-forward · config-attribution · Thompson-bandit · a promóciós kapu hard „beats market" gate-je), **18 új assert**. **Élőben:** a jelenlegi (pre-B53) ledgeren a számok **bit-azonosak** → a fallback ép, a fix tisztán forward-tölt. **Nem trading-hiba** (a bot mindig valós áron kereskedett). Doksi: [math/21 §6](internal-docs/math/21-walk-forward.md).

**B52 4. lépcső — 4 → 8 ensemble-rendszer.** A nyitott kérdés („kell-e per-állomás lista?") megválaszolva: **nem** — a domainjén kívüli regionális modellt az API **némán elhagyja** (Tokió `gfs_seamless,icon_eu` → HTTP 200, 31 tag), csak önmagában kérve ad 400-at. Új: `gem_global` (21) + `ukmo_global_ensemble_20km` (18) mindenhol, `icon_eu` (40 @ **13 km**) + `icon_d2` (20 @ **2 km**) Európában. **Európa 8 rendszer/296 tag, máshol 6/236**, ugyanazon a költségen (**1 kérés, ~42 KB, ~0,2 s**). Kihagyva: `bom_access_global_ensemble` (csupa null minden állomáson) és `ukmo_uk_ensemble_2km` (3 tag). Élőben: Londonban a két ICON ~**1 °C-kal melegebb** a globálisoknál — pont az a lokális jel, amit a 25 km-es rács elsimít; a legacy trading-út **bit-azonos**. Doksi: [math/37 §2](internal-docs/math/37-multi-model-ensemble.md) · [changelog 2026-09-09](internal-docs/changelog/CHANGELOG-2026-09-09.md) · [sprints B52/B53/B54](internal-docs/roadmap/sprints.md).

**Sports (92. session, 2026-09-09 — a user megerősített döntése):** a bot **FUT** (paper, `stopped=false`, cron ON), de **ki van zárva a cross-bot aggregátumból**. ⚠ **Történet:** a 86. sessionben a user döntése alapján futott + nullázva lett; a 90. session rendszer-audit **leállította** (`sportsCronEnabled=0` + manuális stop, a régi B37-politika alapján); a **napi drift-ellenőrzés első futása kiszúrta**, és a user megerősítette a reggeli döntést → visszaindítva (bankroll $50 / 0 trade, cron-override törölve). **Két session ellentétesen döntött ugyanarról** — ha a politika változik, ITT kell frissíteni, különben újra szétcsúszik. Ok a kódból: odds-feed (B37) nélkül a fair-value `predicted = 0.5 + (yesPrice − 0.5) × 0.55` — annak az árnak a determinisztikus transzformációja, ami ellen fogad (a pillérben 0 külső adatforrás), így **minden belépő longshot, aritmetikából**. A skillje konstrukcióból ≈0, viszont **118/236 rezolvált ledger-sort** adott = a promóciós kapu bizonyítékának fele → érzéstelenítette a mutatót. Új `AGGREGATE_EXCLUDED` az [`edge-tracker.mts`](services/api/src/routes/edge-tracker.mts)-ben: a `category="all"` pool kihagyja; a **saját fülje és a ledger-gyűjtése változatlan**. Session **nullázva** ($50 / 0 trade), a ledger megtartva. ⚠ A fix **rosszabbnak** mutatja az aggregátumot (ALL Brier-skill −52,78% → **−135,31%**) — ez a helyes: eddig a sports maszkolta, milyen rosszul áll a crypto+weather. **B37-nél visszavonandó.**

**Következő:** deploy → a B53 tiszta mérése + a 8-rendszeres log innen indul → 2-3 hét után `bun scripts/eval-multimodel.ts` (B52 flip) és a **B54** knob-kiértékelés valódi A/B-karokkal.

### Korábbi munka (83. session, 2026-09-08) — B52 1. lépcső: weather multi-model log-forward (mérés-only, 0 trading-hatás)

A user: „csináld meg a B52 első lépcsőjét." **Implementálva** (`tsc` exit 0 + **48/48 teszt** [3 új suite, 91 új assert] + build zöld, `main`, **NEM deployolva**). A bot ma **egyetlen** ensemble-családot használ σ-forrásnak (31-tagú NCEP GEFS) — egy ensemble szórása viszont a saját perturbációié, a **strukturális** modellhibát nem látja. Ez a túl szűk σ méretezi túl a Kellyt (a dokumentált `forecast_edge` IC **+0.393** = jó irány / payoffRatio **0.44** = rossz méret).

**Amit szállítottam:** **(1)** pure core [`@core/multi-model-ensemble.mts`](packages/core/src/multi-model-ensemble.mts) — `splitEnsembleKeys` a modell-taget a **válasz-kulcsból** olvassa (az Open-Meteo a belső domain-nevet adja: `gfs_seamless` → `ncep_gefs_seamless`; aliasra illesztve némán 0 tagot találna) + `modelStatsForDate` **modellenként egyenlő súlyú** keverékkel (σ²=mean(σ²_k)+var(μ_k) — tagonkénti súly a 64-tagú WN2-nek kétszeres szavazatot adna a 31-tagú GEFS fölött) + [47 assert](packages/core/src/multi-model-ensemble.test.mts). **(2)** [`fetchMultiModelEnsemble`](services/worker/src/pillars/weather/ensemble-forecast.mts) — **1 kérés / 197 tag / ~30 KB / ~0,2 s** (mérve), `daily=temperature_2m_max` (numerikusan azonos a hourly-derivált maxszal). **(3)** [`multi-model-store.mts`](services/worker/src/pillars/weather/multi-model-store.mts) — (állomás, céldátum) párra ~3 órás throttle (`dueForSnapshot`, a B50 #2 primitívje), gördülő 400 snapshot; minden rekord rögzíti a per-rendszer μ/σ-t **és amit a bot azon a tickben ténylegesen használt** → azonos pillanatban vett fej-fej; az `obs` az EMOS-store-ból másolódik (**nincs második METAR-kör**); [13 integrációs assert valós Postgres (PGlite) ellen](services/worker/src/pillars/weather/multi-model-store.test.mts). **(4)** read-out [`scripts/eval-multimodel.ts`](scripts/eval-multimodel.ts) + [`@core/multi-model-eval.mts`](packages/core/src/multi-model-eval.mts) — **CRPS-skill** + **var-ratio** (`mean(hiba²)/mean(σ²)`: >1 = underdispersed) + coverage, explicit verdikttel; a [31 assert](packages/core/src/multi-model-eval.test.mts) pineli, hogy *azonos pont-hiba mellett a becsületes σ jobb CRPS-t kap* (egy változat rosszabb MAE-vel is nyerhet — a probléma a méretezés, nem az irány).

**Knobok:** `weatherMultiModelRecord` **default 1** (csak logol; azért ON, mert az összehasonlítás **nem backfillelhető** — a WN2 historikus archívum csak ~2026-09-04-től él, az AIFS-ENS-nek nincs → B50 #2 doktrína, és illik a B49 #6 EMOS „apply OFF / log ON" mintájához) + `weatherUseMultiModel` **default 0** (a flip). Env: `WEATHER_MULTIMODEL_RECORD` / `WEATHER_ENSEMBLE_MODELS` / `WEATHER_MULTIMODEL_INTERVAL_MIN` / `WEATHER_USE_MULTIMODEL`.

**0 trading-hatás — élőben verifikálva** 4 állomáson: a `predictedMaxC`/`confidence`/`modelUsed` a bekapcsolt recorder-fetch-csel **bit-azonos** a recorder nélküli hívással. **Tokió (2026-09-09 célnap) egy sorban:** GEFS μ 33.97 **σ 1.02** · IFS-ENS 30.80 · **AIFS-ENS 28.78** · WN2 29.16 → keverék μ 30.68 **σ 2.21** (inter-modell 2.05). A GEFS **5,2 °C**-kal az AIFS fölött ül, miközben ±1,02 °C-ot állít magáról. Londonban viszont a keverék σ **szűkebb** (0.84 vs 0.91) → nem uniform tágítás, hanem **helyesebb** σ. ⚠ A flip után **kevesebb** trade lesz (confidence = 1 − σ/4 → a `weatherConfidenceMin` 0.65 többet blokkol; Tokió kiesne) — ez a szándék. Doksi: [math/37](internal-docs/math/37-multi-model-ensemble.md) · [changelog](internal-docs/changelog/CHANGELOG-2026-09-08.md) · [sprints B52](internal-docs/roadmap/sprints.md). **Következő:** deploy → ~2 nap múlva első címkézett sorok → 2-3 hét után `bun scripts/eval-multimodel.ts` → verdikt alapján flip.

### Korábbi munka (82. session, 2026-09-08) — Edge Tracker mobil-overflow fix + weather multi-model felmérés

A user: „mobilon két komponens az edge tracker oldalakat eltöri és széltében elmegy a nézet! javítsd" + „a weather botba be tudnánk integrálni weather api-kat vagy modelleket? elvileg a gemini is pontosított a modelljén".

**(1) Mobil-fix — reprodukálva és javítva** (`tsc` 0 + **45/45 teszt** + build zöld, `main`). Lokális dev-szerver + 375 px emulált viewport + mockolt edge-tracker-válasz: `clientWidth 375` vs `scrollWidth 496` = **121 px túlcsordulás**; a két törő kártya a `WinRateHeatmap` és a `PnlDistribution`. **Gyökérok — egy hiba, két tünet:** a `.et-heatmap` `grid-template-columns: 60px repeat(24, 1fr)`-je ~434 px min-contentű, és mivel az `1fr` sáv automatikus minimuma a min-content, az `.et-grid2` egyoszlopos mobil-sávja **a szomszéd kártyát is** 468 px-re feszítette. Fix az [`EdgeTrackerPanel.tsx`](apps/web/src/components/EdgeTrackerPanel.tsx)-ben: `.et-grid2 > * { min-width: 0 }` (szerkezeti blowout-guard) + a heatmap saját `.et-heat-scroll` vízszintes scrollere `min-width: 440px`-szel mobilon (olvasható cellák, swipe — a projekt `.tbl-scroll` konvenciója) + **sticky kategória-oszlop** gap-átfestő box-shadow-val. Verifikálva **0 túlcsordulás 320 és 375 px-en, desktop bit-változatlan** (a heatmap 485 px az 519 px-es sávban, nem görget). Mellékesen: [`.claude/launch.json`](.claude/launch.json) új `web-dev` bejegyzés (Astro dev, 4321).

**(2) Weather model-bővítés — felmérve, kód NEM változott.** A bot ma **csak a 31-tagú GEFS-et** használja ensemble-nek (`models=gfs_seamless`). Élő-verifikálva ugyanazon a keyless endpointon: `models=gfs_seamless,ecmwf_ifs025,ecmwf_aifs025,google_weathernext2_ensemble` → **197 tag egyetlen kérésben** (31 + 51 ECMWF IFS-ENS + 51 ECMWF **AIFS**-ENS + **64 Google DeepMind WeatherNext 2**) — vagyis a math/16 §3.B (a) opciója (ECMWF-kulcs) **tárgytalan**. **Mért indok** a bot saját állomásain T+0/T+1 daily-maxra: a GEFS-only σ **×2–3-mal alulbecsli** a bizonytalanságot (Tokyo T+0: σ 0.49 → **1.58**, μ 32.3 → **29.3**; HK T+1 ×2.5; Chicago T+0 ×2.3), Európában viszont *szűkebb* (London ×0.7) → nem tágítás, hanem **helyesebb** σ. Ez a dokumentált „`forecast_edge` IC +0.393 = jó irány / payoffRatio 0.44 = rossz sizing" patológia gyökere. **Flip előtt mérni kell:** a WN2/AIFS natívan 6-órás (a napi max csúcsa simulhat; az AI-modellek hideg-biasosak hőhullám-csúcson, arXiv 2504.21195), **nincs offline backfill** (a `historical-forecast-api` WN2-t csak ~2026-09-04-től ad → **log-forward MOST**, B50 #2), és az Open-Meteo free tier **non-commercial + hívás-súly a változószámmal nő**. WeatherNext **3** (5 km, órás station-head) még nincs az Open-Meteo-n. Doksi: [changelog 2026-09-08](internal-docs/changelog/CHANGELOG-2026-09-08.md) · [math/16 §3.B (d)](internal-docs/math/16-weather-bot.md) · **[sprints B52](internal-docs/roadmap/sprints.md)** (a régi B4 felváltva). **Következő:** B52 1. lépcső — per-modell μ/σ log-forward az `emos-store`-ba (0 trading-hatás), 2-3 hét után DEB/EMOS-alapú flip.

### Korábbi munka (81. session, 2026-09-08) — B51: crypto multi-coin scan (BTC-only → BTC/ETH/SOL)

A user: „állítsuk lejebb a küszöböt hogy több trade/adat legyen, vagy az rossz irányba vinné a modellt? a crypto alig kereskedik" → „nézd meg" → „bővíthető-e több strike/coin-ra, mikor kalibráljunk?" → „mérd fel a coin-bővítést, sprintbe, végezd el, ellenőrizd." **Élő diagnózis (boxon):** a crypto fut, de 2h alatt **0 nyitás / 120 skip** — nem a küszöb a szűk keresztmetszet, hanem **piac-kínálat** (BTC a 78k strike-on ült → 3 piac, mind coinflip; a resolution-risk + confidence gate helyesen blokkol). A küszöb-lazítás nem hatna a gate-blokkolt piacokra + coinflip-zajt hozna. **A helyes kar: több coin** (a B49 #5 „BTC-hardcode leváltása" célja). **Implementálva** (`tsc` 0 + **45/45 teszt** + build zöld, `main`): új SSOT [`@core/coin.mts`](packages/core/src/coin.mts) (coin-detektor + strike-parser: BTC „k"×1000 / ETH literál, coin-scoped closingKey; a 3 duplikált regex egyesítve) + [teszt](packages/core/src/coin.test.mts); a [finder](services/worker/src/pillars/crypto/btc-market-finder.mts) `CRYPTO_COINS` allowlist-tel (default `BTC,ETH,SOL`) + `MarketInfo.coin` tag; a [combiner](services/api/src/routes/signal-combiner.mts) **7 BTC-hardcode-olt fetch-e coin-aware** (spot/σ/funding/OI/Deribit-currency/cond_prob coin+K); az [aggregator](services/worker/src/pillars/crypto/signal-aggregator.mts) OB-imbalance coin-aware; a [gates](services/worker/src/pillars/shared/cross-position-gates.mts) coin-scoped. **BTC bit-azonos** (default-BTC fallback → 0 regresszió); ETH/SOL a deploy után élesedik. Doksi: [changelog 2026-09-08](internal-docs/changelog/CHANGELOG-2026-09-08.md) · [math/36](internal-docs/math/36-multi-coin.md) · [sprints B51](internal-docs/roadmap/sprints.md). **Kalibráció-ütem (a user kérdésére):** ~2-3 hét (≥30 rezolvált predikció/kategória → Promotion Gate), a most bekapcsolt 10 knob kiértékelése ~2-4 hét forward-adat után config-attribution + Brier-skill alapon. **Következő:** deploy-verifikáció (ETH-piacok a scan-en), majd az adat-gyűlés.

### Korábbi munka (80. session, 2026-09-04) — B50 operatív aktiválás (10 knob ON) + élő trade-audit → oi_delta fix

A user sorban: „csináld meg A-t" → majd „bekapcsolhatod a knobokat" → „nézd meg a trade-eket." **Boxon (`ssh analytics`), verifikálva:** **(1) A-feladatok:** `RECORD_OI=1`+`RECORD_CLOB_BOOK=1` a `.env`-be (recorderek ON, valós OI+könyv gyűlik), weather EMOS seed lefuttatva (**27/27 állomás fittelve**, 4887 residual, CRPS pl. London 0.330→0.307). **(2) 10 knob bekapcsolva** a `blob_kv`-ben (jsonb-merge, a meglévő 2-t megőrizve; +DSR-trial logolva) — a harness-classifier blokkolta a saját DB-írásom, ezért a user futtatta a kész SQL-t, majd bash-engedélyt adott. **(3) Élő trade-audit** (a user kérésére): weather PnL bit-pontos, fill-modell él (VWAP>touch slippage), HL PnL a `payload`-ban (bankroll bit-rekonciliál 200.16), config-stamp forward-tölt, 0 hiba (az egy skip helyes guardrail). **Bug: az `oi_delta` a combinerben számol + hat a finalProb-ra, de a worker [`signal-aggregator.mts`](services/worker/src/pillars/crypto/signal-aggregator.mts) `extractBreakdown`-ja kihagyta → a ledger-breakdown NULL → a per-signal IC nem trackelt. Fix pusholva** (`34d0d3b`, `oi_delta` mindkét builderbe; `tsc`+44/44+build zöld). Doksi: [changelog 2026-09-04](internal-docs/changelog/CHANGELOG-2026-09-04.md). **Következő:** pár nap múlva Edge Tracker (Promotion gate/Config bandit) → a knobokat egyesével meghagyni/visszavenni; user-oldal: B37 odds-api kulcs, live-flip (B10).

### Korábbi munka (79. session, 2026-09-03) — a B50 training-commitok (#1–#9) audit-passza → 5 fix
