...
**Élő deploy:** `mj-trading.netlify.app`. Paper mode, simVersion 3 (crypto), v2 (HL). **A live rendszer még Netlify** (a `main` branchen) — a lenti Hetzner-migráció külön `feat/hetzner-migration` branchen zajlik, strangler-módban (a Netlify érintetlen a cutoverig).

### Legutóbbi munka (54. session, 2026-09-02) — Hetzner-migráció Phase 0 + Phase 1 (repo → monorepo)

A user kérte a [migration-runbook.md](internal-docs/roadmap/migration-runbook.md) végrehajtásának indítását (Netlify+Blobs → `analytics` Hetzner Docker co-host), Phase 1-től. **Ez a session: Phase 0 (branch + secret-leltár) + Phase 1 (repo-restrukturálás) — kód-LOGIKA nem változott, csak fájlszerkezet + import-útvonalak.** Új branch **`feat/hetzner-migration`** (a forecasting-branchről ágazva, NEM merge-elve main-be). **Phase 0:** teljes `process.env` scan → grouped `.env.example` (secret+infra+mode kötelezők; a knobok kód-defaulttal opcionálisak); új infra-secret `EDGECALC_DB_PASSWORD`+`DATABASE_URL`. **Phase 1 (mind a 3 acceptance-gate zöld):** `git mv`-vel (176 rename, history megőrizve) → `apps/web/` (a teljes Astro app, 0 frontend churn), `packages/core/src/` (9 pure modul + shared `types.mts` + a 10 forecasting teszt-suite; a `prediction-ledger` egyelőre még `@netlify/blobs` — a csere Phase 2), `services/worker/src/pillars/` (a teljes `auto-trader/` fa, belső struktúra megőrizve), `services/api/src/routes/` (~25 endpoint + auth-guard + mock-trades). Import-repair **path-aliasokkal** (`@core`/`@worker`/`@api`) + `tsc`-vezérelt iteratív fix (105→0 TS2307); a `tsx` futásidőben is honorálja az aliasokat. Új `scripts/run-tests.mjs` + root monorepo-scriptek + `.dockerignore`. **Acceptance:** `tsc --noEmit` exit 0 · `packages/core` 10/10 (teljes 23/23) teszt zöld · `apps/web` astro build zöld. **Ismert Phase-later:** `netlify.toml` ezen a branchen elavult (a main deploy-ol, strangler) → Phase 5/6; per-service Dockerfile+workspaces → Phase 3; api↔worker cross-import átmeneti layering → Phase 3. Részletek: [changelog 2026-09-02](internal-docs/changelog/CHANGELOG-2026-09-02.md). **Következő: Phase 2 — `packages/core` Blobs→Postgres adapter.**
