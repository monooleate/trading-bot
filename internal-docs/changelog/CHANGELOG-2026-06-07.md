...
- **Reset VÉGREHAJTVA (2026-06-07 04:54 UTC):** mind az 5 bot tiszta lapra (auth-olt API, login + `action=reset` per kategória, bankroll-start megőrizve: crypto $350 / weather $250 / HL $200 / F-Arb $200 / sports $450). Élő status megerősítve: minden bot `closed=0, sessionPnL=0, bankrollCurrent=bankrollStart, startedAt=04:54Z`. A `multi-status` átmenetileg elavult run-state cache-t mutatott (a per-bot `?action=status` az SSOT, az frissült azonnal). A botok a következő cron-tickektől (≤3 perc) az új gate-ekkel (B23 shrink @0.5 + sports floor @0.05) gyűjtenek friss adatot.
