...
## B50 #6 + #8 — diszkontált Thompson-sampling config-választó implementálva (75. session), mérés-only

A user: „menj a #6-ra." A discovery egyetlen erős új adaptív technikája: a rendszer diszkrét configok közt választ bizonytalanság + késleltetett zajos reward mellett → bandit-probléma. A Thompson-sampling a helyes, minta-hatékony válasz. **Kód a `main`-en, NEM deployolva.** `tsc` exit 0 + **43/43 teszt** (+ thompson) + build zöld.

**Implementálva:**
- **Pure modul** [`packages/core/src/thompson.mts`](../../packages/core/src/thompson.mts): **#8** `forgettingWeight(age, halfLife)` = `0.5^(age/halfLife)` (a megosztott decay-primitív; halfLife ≤ 0 → 1); `betaPosteriors` (config-onként diszkontált `Beta(1+Σw·r, 1+Σw·(1−r))` → mean, nEff, nRaw); `thompsonRank` (**prob-best Monte Carlóval:** fix-seed LCG → Box-Muller normál → Marsaglia-Tsang gamma → Beta-minta, reprodukálható; prob-best-desc); `banditArmsFromRecords` (a config-attribuált ledgerből: configHash-arm-ok, reward = model-Brier < piac-Brier, age = kronológiai rang a legújabbtól, explicit null-guard). 6-csoportos [teszt](../../packages/core/src/thompson.test.mts).
- **Reward:** proper-score-származék (model verte-e a piacot) — nem nyers PnL (kisebb variancia, nehezebb gamelni).
- **Bekötés** [`edge-tracker.mts`](../../services/api/src/routes/edge-tracker.mts): `banditEval` response-mező (a betöltött ledger-rekordokból); a **#8 fél-életidő az `icHalfLifeTrades` knobból** (>0), különben default 75 rezolvált predikció. UI: új **`BanditEvalCard`** ([`EdgeTrackerPanel.tsx`](../../apps/web/src/components/EdgeTrackerPanel.tsx)) — config-onként prob-best sáv + win-rate + effektív-n; ≥90% prob-best → favorit-jelzés.

**A bandit JAVASOL** — a győztes **alkalmazása** (champion-challenger, bounded lépés, a risk-limiteket sosem érintve) **gated follow-up.** A #4 config-stampek a deploytól forward-töltenek → kezdetben az „unlabeled" arm dominál, ahogy a config-címkézett predikciók gyűlnek, a bandit élesedik. **0 trading-hatás.** Ortogonális kiegészítés az AdaHedge-hez (az a jel-súlyokat hangolja online, ez a diszkrét config-választást). Doksi: [`math/33-thompson-bandit.md`](../math/33-thompson-bandit.md). **Follow-up:** #8 továbbvitel (Platt + realized-IC ugyanazt a `forgettingWeight`-et); #6 live-fele (champion-challenger shadow-promóció a #1 kapun át). RL alfára = tiltva (discovery). Tracker: sprints.md B50 #6/#8 ✅.

## B50 #7 — regularizáció-budget (holt-knob audit + plateau-not-peak) implementálva (76. session)

A user: „menj a #7-re, aztán a #9-re." A #7 magja a holt-knob audit + a tunable-felszín szűkítése. **Kód a `main`-en, NEM deployolva.** `tsc` exit 0 + teszt + build zöld.

**Audit-eredmény:** a ~96 SCHEMA-knob mind bekötve fogyasztódik (a mechanikus kereszthivatkozás a `config.mts` effective-mapperen + `signal-combiner`-en + pillér-logikán át **0 teljesen holt knobot** talált; a korábbi egyetlen holtat — `icHalfLifeTrades` rossz objektumról — a 67. session-audit már javította) → **nincs törölnivaló.** **De 96 dimenzió túl sok a mintához** (False Strategy Theorem / MinTRL) → a regularizáció **fegyelem, nem törlés.**

**Knob-osztályozás (a budget):** **A. risk-guardrail** — SOHA nem tuning-tárgy, a #6 bandit sem érinti (Kelly-frakció, loss-limitek, DD-kill/vol-target, leverage, edge-cap, max-open, paper-never-stop, live-ready-override); **B. tunable** — kevés, nagy-karú, plateau-sweep szabad (confidence-min, edge-thresholdok, Kelly-scale-ek, calibrationShrinkageK, icHalfLifeTrades, domén-metódus-flagek: weatherUseEmos/sportsUsePinnacle/useHarRv/useDeribitIV/combinerLogOdds/Extremize); **C. fix theory-default** — a hosszú farok, NE sweepeld (bonferroni, collinearity, ob-imbalance, min-active-signals, cooldownok, min-price/band, watch-extreme-edge, market-disagree, fr* a Sprint-47 után, btc-ablakok, exit-before-min).

**Fegyelem-eszköz:** új pure modul [`packages/core/src/plateau.mts`](../../packages/core/src/plateau.mts) — `selectPlateau(candidates, {tol})` (a **legszélesebb** összefüggő near-best érték-futam **közepét** választja, nem a csúcsot; `isPeak`-jelzés izolált tüskére — a plateau robusztus OOS, a peak a túlillesztés ujjlenyomata) + `ensembleWeights(candidates, temperature)` (softmax → átlagolás a plateau fölött, szigorúan robusztusabb, mint egyet választani). 5-csoportos [teszt](../../packages/core/src/plateau.test.mts). **Munkafolyamat:** egy B-knobot néhány értéken sweepelsz (paper) → a #4 config-attribúció (edge-tracker) adja a per-config Brier-skill score-okat → `selectPlateau` a robusztus érték (`ensembleWeights` az alternatíva) → a promócióhoz a #1 kapu + #3 effektív-DSR.

**Nincs kód-törlés** (a felszín bekötött) — a deliverable a budget-fegyelem (doksi) + a plateau/ensemble helper (kód). Doksi: [`math/34-regularization-budget.md`](../math/34-regularization-budget.md). Tracker: sprints.md B50 #7 ✅.

## B50 #9 — ledger-lefedettség (sports-ledger + deferred infra) implementálva (77. session)

A user: „menj a #7-re, aztán a #9-re." A #9 három részének őszinte scope-ja (a kód-valóság szerint). **Kód a `main`-en, NEM deployolva.** `tsc` exit 0 + **44/44 teszt** + build zöld.

- **(a) Sports prediction-ledger BEKÖTVE:** a sports bináris Polymarket-piac, ami rezolvál (mint crypto/weather), de eddig **semmi** ledger nem volt rá. [`sports/index.mts`](../../services/worker/src/pillars/sports/index.mts): `appendPredictions("sports", results, markets, session.closedTrades, undefined, cfgHash)` + `reconcileLedger("sports")` a `saveSportsSession` után, a config-fingerprinttel stampelve (#4), best-effort (sosem töri a ticket). [`edge-tracker.mts`](../../services/api/src/routes/edge-tracker.mts) `LEDGER_CATEGORIES` += `"sports"` → a sports is kap proper-scoring / walk-forward / config-attribúció / bandit szubsztrátumot.
- **(b) Ledger → normalizált Postgres-tábla: DEFERRED (B12).** A ledger MÁR Postgres-en van (a blob_kv KV-shim → `blob_kv` tábla, JSON). A normalizált `prediction_ledger` tábla (a `ledger.ts` létezik, nincs bekötve) csak SQL-queryability nicety — funkcionálisan nem szükséges, a migráció koordinált worker+api + adat-shape kockázat. B12-re marad.
- **(c) HL / F-arb ledger-baseline: NOT-APPLICABLE.** F-arb delta-neutrális carry → nincs P(YES) forecast, amit scoreolni lehetne. HL perp directional → nincs bináris piaci ár / rezolúció, a walk-forward „beat the market price" nem alkalmazható; a HL ledger a természeténél fogva taken-only (a kimenet a realizált PnL, nem market-rezolúció).

**Állapot:** a sports jelenleg leállítva (fabrikált fair-value + B37 odds-feed-hiány) → a runner a stopped-guardnál korán kilép, tehát a ledger a sports újraindulásáig (B37 után) nem tölt; de az infra kész (post-B37 valós Shin-fair-value-kat logol). **Ismert korlát (follow-up):** a fő edge-skip ág (`!decision.shouldTrade`) nem hordoz `predictedProb`-ot (a `predicted` a skip után áll elő, a `decision` nem exponálja) → a sports-ledger egyelőre taken-heavy; a teljes unbiased skip-loggolás a decision-engine-exponálást + B37-et igényli. Doksi: [`math/35-ledger-coverage.md`](../math/35-ledger-coverage.md). **→ A B50 A-lépcső (#1–#9) TELJES** (a deferred/N-A részek explicit indoklással). Tracker: sprints.md B50 #9 ✅.

## B50 operatív aktiválás (78. session) — recorderek ON + weather EMOS seed lefuttatva a boxon

A user: „csináld meg A-t" (a felhalmozott box-akciók). **Élesben a boxon (`ssh analytics`), verifikálva:**
- **A1 — recorderek (#2) bekapcsolva:** `RECORD_OI=1` + `RECORD_CLOB_BOOK=1` a box `.env`-jébe fűzve (backup `.env.bak.*`; a deploy rsync a `.env`-et kizárja → túléli a deployokat), `docker compose up -d --no-deps workers` (rebuild nélkül — a recorder-kód már az image-ben). **Verifikálva:** az első tick valós adatot írt a `blob_kv`-be — `oi-BTC` (112 091 kontraktus ~$9.16B), `oi-ETH`, `oi-SOL`, `clob-book` (a nyitott `bitcoin-above-80k-on-september-4-2026` piac valós L2-mélysége). Innentől minden tick-en logol.
- **A2 — weather EMOS seed (#5) lefuttatva:** a `scripts/seed-emos.ts` nincs az image-ben (`scripts/` nem másolt) → `docker cp` a konténerbe + `docker exec -u root -w /app … bun scripts/seed-emos.ts 6`. **Mind a 27 állomás fittelve** (`fitted=true`), 181 residual/állomás, összesen **4887 pár**. Verifikálva: EGLC (London) `a=-0.25, b=1.02, c=0.25, d=0.64`, **CRPS 0.330→0.307** (a kalibráció javít). A `weatherUseEmos=1` bekapcsolásakor az 1. naptól kalibrált μ,σ.

**A default-OFF forecasting-knobok (weatherUseEmos/sportsUsePinnacle/useHarRv/… + fill-modell/beta-cap/OI-Δ) továbbra is OFF** — az élesítés az Edge Tracker promóciós-kapu (#1) / config-bandit (#6) pozitív bizonyítéka után. Memória frissítve: `hetzner-box-deploy` (auto-deploy Option-B rsync + recorder/seed állapot). A box `git HEAD` rsync-deploy miatt elavultnak látszik (3ff6646), de a fájlok/konténer a legújabb kódot futtatják.

## B50 audit-passza (79. session) — a #1–#9 training-commitok audit-ja → 5 fix (measure-only-safe)

A user kérése: **validáld és auditáld az előző audit (67. session, `d2143f5`) óta eltelt commitokat** — a „trading bot optimization" (B50 training) tevékenységet — megvalósításra és biztonságra, a hibákat javítsd, majd push a `main`-re.

**Módszer:** zöld baseline (`tsc` 0 + 44/44 teszt) rögzítése, majd **4 párhuzamos read-only audit-ágens** a `d2143f5..HEAD` diffen (50 fájl, ~3700 sor: a B50 #1–#9 pure-core modulok + worker/api wiring + deploy.yml + UI). A deploy.yml rsync-`--delete`-et magam is verifikáltam (nincs tracked `data/`/`logs/`/`dist/` source-dir → az exclude-ok biztonságosak).

**Verdikt: 0 P0/P1.** A security- és UI/contract-audit tiszta (csak P3 nitek); a core-math korrekt (FNV-1a, union-find, Marsaglia–Tsang gamma→Beta, Corsi HAR OLS, multiclass Brier, softmax mind textbook-helyes). **1 P2** (sports-ledger) + néhány P3 javítva. Mind: `tsc` exit 0 + **44/44 teszt** + build zöld, `main`.

### Fixek

- **P2 — sports-ledger (B50 #9) gyakorlatilag inert volt** ([`sports/index.mts`](../../services/worker/src/pillars/sports/index.mts)): a `reconcileLedger("sports")` be volt kötve, de **egyetlen result-row sem hordozott `endDate`-et** → `buildIncoming`/reconcile pending-szűrője (`r.endDate && …`) sosem talált jelöltet → a skipped-market Gamma-reconcile strukturálisan **sosem futott**. Ráadásul a **skip-rowok `predictedProb` nélkül** mentek → `buildIncoming` eldobta őket → a ledger nem „taken-heavy", hanem **taken-only** volt. **Bónusz-bug:** a *traded* row `predictedProb`-ja `predicted` (= P(chosen side)) volt, ami NO-trade-eknél **hibás** a ledger-kontraktushoz (`predictedProb = model P(YES)`, direction-agnosztikus) képest → rossz Brier a Gamma YES-rezolúció ellen. **Fix:** direction-agnosztikus `yesProb = clamp(0.5 + (yesPrice−0.5)·0.55)` a loop-tetőn, minden row-ra (traded + mind a 4 skip-ág) `predictedProb: yesProb` + `marketPrice: m.yesPrice` + `endDate: m.endDate`. Zero UI-hatás (a `SportsTrader` nem olvassa a result-row `predictedProb`-ot; a rationale-t az `entryDecision.finalProb` adja, változatlan). Regressziós pin: [`prediction-ledger.test.mts`](../../packages/core/src/prediction-ledger.test.mts) „sports-ledger" blokk. **Feloldja** a math/35 „ismert korlát"-ját (a fabrikált fair-value **minősége** külön ügy → B37).
- **P3 — `devig-eval.mts` sort-guard:** `(a.brier || Infinity)` egy legitim `brier===0`-t is a végére sorolt (falsy). Fix: `Number.isFinite`-guard.
- **P3 — `devig-eval.test.mts` nem pin-elte a metódus-különbséget:** minden rekord azonos odds-szal → egy `devigShin`→`multiplicative` regresszió is átment volna. Új **3b. „flb-direction"** teszt: a `devigMultiplicative`/`Power`/`Shin` közvetlen összevetése — a longshot LEFELÉ, a favorit FELFELÉ húzva + `shin ≠ multiplicative` (a modul létjogosultsága most pin-elt).
- **P3 — `config-fingerprint.mts` némán ejtette a nem-number override-okat:** egy numerikus string (`"1"`) kiesett volna a hashből → két különböző config **kollíziója**. Fix: number VAGY nem-üres numerikus string elfogadva, a parsed számra kulcsolva (bool/null/objektum továbbra is kizárva). No-op a jelenlegi (mind-number) adaton.
- **P3 — `har-fit.mts` `trainFrac` nem volt clamp-elve:** `trainFrac ≥ 1` → üres OOS-ablak → `n=0` → NaN-MSE-k az `empty` sentinel helyett. Fix: clamp `[0.1, 0.95]` + `split ≤ N−1`.

**NEM javítva (tudatosan):** a `har-fit` OLS a kollineáris HAR-design-on regularizáció nélkül (P2, statisztikai) — **measure-only offline-validáció** (a `fit-har.ts` script, nem live-wired), OOS-guarddal (rossz fit egyszerűen veszít az equal-weight ellen) + a header dokumentálja; egy ridge-tag megváltoztatná a changelogban validált R²=0.044 számot → hagyva. A P3 UI-nitek (index-as-key, single-config bandit copy, gate-on-mock) kozmetikaiak. A deploy.yml P3-ak (exclude leading-slash, TOFU host-key) a single-maintainer/paper kontextusban nem valós kockázat.

**Deploy:** push `main` → CI (tsc+teszt+build) → a `deploy.yml` a boxra rsync-eli. A sports **leállítva marad** (B37-ig), így a ledger-fix a sports újraindulásakor lép életbe; a fix strukturális (0 trading-hatás, measure-only). A box `.env` recorder/seed-állapota változatlan (a rsync kizárja).
