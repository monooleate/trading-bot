...
### B34 — Combiner sign-aware negatív-IC kezelés + `useRealizedIC` bekapcsolása 🟠

- **Precondition:** B30 (clamp) élesítve — **kötelező előfeltétel**, e nélkül a knob bekapcsolása out-of-range valószínűséget adna. **Becslés:** 1–2 nap (rewrite + teszt) + knob.
- **Feladat:** a `combine()` directional/HL piacokon is kapjon **sign-aware log-odds** korrekciót (a threshold K-anchor tilt-ág mintájára [signal-combiner.mts:1229](../../netlify/functions/signal-combiner.mts)): negatív-IC jel **invertálja** a hozzájárulását (`1−p`), ne csak zsugorítsa a súlyt. Utána `useRealizedIC=1` bekapcsolás crypto+HL-en (Bayes-shrinkage, K=30), miután minden jel n≥30 closed trade — így az `orderflow` (−0.165), `vol_divergence` (−0.32), `pairs_spread` (−0.13) automatikusan lesúlyozódik/invertálódik. **Várt hatás:** a HL ~breakeven → enyhén pozitív; a crypto evGap (−$454) csökken.

### B35 — Weather sizing/kalibráció overhaul (payoffRatio 0.44 gyökérok) 🟠
