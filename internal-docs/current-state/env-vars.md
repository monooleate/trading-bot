...
| `WEATHER_ENSEMBLE_MODELS` | 6 globális + 2 európai regionális (a lista: [`@core/multi-model-ensemble.mts`](../../packages/core/src/multi-model-ensemble.mts)) | A multi-model fetch modell-listája (vesszővel). **Egyetlen** Open-Meteo kérés: Európában 8 rendszer / 296 tag, máshol 6 / 236 (~42 KB, ~0,2 s) → 0 extra HTTP-hívás. A regionális modelleket (`icon_eu`, `icon_d2`) az API a domainjükön kívül **némán elhagyja**, ezért egy lista minden állomásra jó. |
...
| `WEATHER_SIGMA_INFLATION` | `1.0` | **P0-1** (rendszer-audit 2026-09-09) — a bucket-matchernek átadott előrejelzési σ szorzója, EMOS UTÁN alkalmazva. `1.0` = KI, **bit-azonos**. Indok: az EMOS-store forward felén (élő METAR obs vs. a ténylegesen használt GEFS-átlag, n=35) `mean(err²/σ²)` = **10,04**, ahol 1,0 a kalibrált → a σ ~1,8× (medián) … ~3,2× (átlag) túl kicsi. A mért plató **2,0** (`selectPlateau`, log-score + CRPS + valós PnL). ⚠ **Kár-csökkentés, nem edge** — monoton transzformáció, a `corr(predikció, kimenet)` = −0,316 előjelét nem javítja. Részletek: [`math/38`](../math/38-weather-dispersion.md). |
| `WEATHER_EMOS_SEED_WEIGHT` | `1.0` | **P0-2** (rendszer-audit 2026-09-09) — a SEEDELT EMOS-residualok súlya az illesztésben. `1.0` = KI, **bit-azonos**. Minden állomás ~181 seedelt (ERA5 obs + inter-modell szórás) és 0–5 forward (METAR + GEFS σ) residualon illeszkedik — két különböző eloszlás (seed bias −0,03 °C vs forward **+0,50 °C**), a seed ~40:1-ben leszavazza az élőt. Leave-one-out mérve: súly 1,0 → CRPS 1,0767; **0,1 → 0,9681**; 0,03 → 0,8623. ⚠ **Párban áll a `WEATHER_SIGMA_INFLATION`-nel** (jobb fit → kevesebb λ kell): seed 0,1 → λ 1,75–2,25. Ajánlott pár: **seed 0.1 + λ 2.0**. Részletek: [`math/23 §6`](../math/23-emos.md). |
| `WEATHER_SHRINK_SIZING` | `0` | **P2-12** — a `selectionShrink` optimizer-curse büntetését a **Kelly-valószínűségre** is alkalmazza, nem csak a trade/nem-trade kapura. `0` = KI (bit-azonos). Eddig egy épp hogy átcsúszó trade **teljes** konvikcióval ment — belsőleg inkonzisztens. ⚠ A `weatherSelectionShrink` a **gyakoriságot** állítja, a méret-knob a `weatherKellyScale`. |
| `CRYPTO_SCAN_WINDOW` | `5` | **B57** — hány crypto piac kerül egy scan-ablakba (coin-diverzifikált slotok, `@core/scan-slots.mts`). Minden slot egy TELJES combiner-futás ~8 külső fetch-csel, tehát ez a bot fő külső-API tárcsája. Settings-knob: `cryptoScanWindow`. |
