...
A net-edge gate-nél a `direction` választása opcionálisan **flippel** (`weatherInvertDirection` knob, default OFF). A 2026-06-04 (n=25) + 2026-06-06 (n=11) flip-auditok szerint a bucket-matcher max-disagreement választása in-sample anti-edge volt, így a modell fade-elése (azonos $ tét) profitált (+$87, +$32). **Band-aid** — a B23 (selection-shrink) a preferált gyökérok-fix; ha az jól sikerül, a B22-t ki kell kapcsolni (különben a jó tippeket fade-elné). A flip a net-edge gate után történik; a cross-position gate az effektív (flippelt) oldalon fut.

**Kelly-méretezés flip alatt (2026-06-13 fix).** A Kelly `probSide`-ot **NEM** a flippelt, hanem a `baseDirection` (modell-preferált, +edge) oldalon számoljuk. Indok: a Kelly a modell konvikcióját méri, ami mindig a `baseDirection`-on él (definíció szerint ott `probSide > priceSide`). Ha a flippelt oldalon méreteznénk, a modell ott sub-fair-value valószínűséget ad → `(probSide·b − q) < 0` → `rawKelly = 0` → `positionSizeUSDC = 0` → `shares = costBasis = 0` → a reconciler minden inverted trade-et `pnl = 0`-val zárna (a 2026-06-13 „invert mode books $0 PnL" élő bug). A `baseDirection`-méretezéssel az invert egy *azonos méretű tükörfogadás* a modell természetes tétjéhez képest — pontosan az apples-to-apples forgatókönyv, amit a flip-audit mért. `invertDirection=OFF` esetén `baseDirection === direction`, így szigorú no-op. → sprints.md B22.
